...
void MainFrame::on_pushLuminanceContrastTransform_clicked()
{
    ...
    KImageColor icOut = icMain.LContrastTransform(ui->spinContrastA->value(), ui->spinContrastB->value(), ui->spinContrastAn->value(), ui->spinContrastBn->value());
    ImageForm* q_pForm = 0;
    ...
    if(q_pForm)
        q_pForm->Update(icOut);
    //새로운 창에 표시
    else
    {
        q_pForm = new ImageForm(icOut, "Contrast Transform", this);
        q_pForm->show();
        _lImageForm.push_back(q_pForm);
    }
    //UI 활성화 갱신
